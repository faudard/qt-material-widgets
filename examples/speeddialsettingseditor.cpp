    ...
    : QWidget(parent)
{
  ...
  m_canvas->setMinimumSize(420, 320);
  m_canvas->setStyleSheet("background: white;");

  m_speedDial = new QtMaterialSpeedDial;
  m_speedDial->addAction(QIcon(), "New");
  m_speedDial->addAction(QIcon(), "Open");
  m_speedDial->addAction(QIcon(), "Save");
  ...
  QVBoxLayout *canvasLayout = new QVBoxLayout(m_canvas);
  canvasLayout->setContentsMargins(0, 0, 0, 0);
  canvasLayout->addWidget(m_speedDial);
...
void SpeedDialSettingsEditor::updateWidget()
{
  ...
  m_speedDial->setAnchorCorner(
    static_cast<Qt::Corner>(m_cornerComboBox->currentData().toInt()));
}